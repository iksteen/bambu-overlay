...
fn current_print_fixture_uses_observed_device_fields() {
    ...
    assert_eq!(device.model_name.as_deref(), Some("BL-P001"));
    assert_eq!(device.product_name.as_deref(), Some("X1 Carbon"));
...
fn tasks_fixture_uses_observed_task_fields() {
    ...
    assert_eq!(task.device_model.as_deref(), Some("X1 Carbon"));
